...
export function MCPModal({ isOpen, onClose }: MCPModalProps) {
  ...
  const getStatusBg = (status: string) => {
    switch (status) {
      case 'connected': return 'bg-green-900/30 border-green-700';
      case 'connecting': return 'bg-yellow-900/30 border-yellow-700';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-gray-900 rounded-xl shadow-2xl w-full max-w-2xl mx-4 max-h-[85vh] flex flex-col border border-gray-700">
        {/* 标题栏 */}
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 text-sm text-purple-400 hover:text-purple-300 border border-purple-700 rounded-lg hover:bg-purple-900/30 transition-colors"
              >
  ...
  "mcpServers": {
    "server-name": {
      ...
      "env": {}
                ...
                className="w-full h-32 bg-gray-900 border border-gray-600 rounded-lg p-2 text-sm text-white font-mono resize-none focus:outline-none focus:border-purple-500"
              />
              <div className="flex justify-end gap-2 mt-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1 text-sm text-white bg-purple-600 hover:bg-purple-700 rounded-lg"
                >
          ...
          <div className="mb-4 p-3 bg-gray-800/50 rounded-lg border border-gray-700">
            <p className="text-xs text-gray-400 mb-2">添加新服务器:</p>
            <div className="flex gap-2">
              <input
                type="text"
                value={newServerName}
                ...
                className="flex-1 bg-gray-900 border border-gray-600 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-purple-500"
              />
              <input
                type="text"
                ...
                className="flex-[2] bg-gray-900 border border-gray-600 rounded-lg px-3 py-1.5 text-sm text-white font-mono text-xs focus:outline-none focus:border-purple-500"
              />
              <button
            ...
            <div className="space-y-2">
              {servers.map((server) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-1">
                      {server.status === 'connected' ? (
                        <button
                          ...
                          className="p-1.5 text-yellow-400 hover:bg-yellow-900/30 rounded-lg transition-colors disabled:opacity-50"
                          title="断开连接"
                        >
                  ...
                  {/* 错误信息 */}
                  {server.error && (
                    <div className="mt-2 flex items-start gap-1 text-red-400">
                  ...
                  {server.status === 'connected' && server.tools.length > 0 && (
                    <div className="mt-2">
                      <p className="text-xs text-gray-500 mb-1">工具 ({server.tools.length}):</p>
                      <div className="flex flex-wrap gap-1">
                        {server.tools.map((tool) => (
                          <span
                            key={tool}
                            className="px-1.5 py-0.5 text-xs bg-gray-700 rounded text-gray-300 font-mono"
                          >
                            {tool}
                          </span>
                        ))}
